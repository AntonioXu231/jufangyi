...
/*
 * ---- Peak-event field map (fixed 2026-09-15) ------------------------------
 * The original dma_s2mm_dds_test.c decoded the peak word with the 10-bit phase
 * map while this build uses PD_PH_FIELD_W = 12 (pd_defines.vh:36), so every
 * phase / pol / ch / seq it printed was wrong by two bits.  v1 inherited that
 * defect; it is corrected here.  The length histogram was never affected --
 * lengths come from S2MM_LENGTH, not from the field decode.  See
 * dma_s2mm_probe_v2.c for the full explanation of the two layouts.
 */
...
static void print_stream_word(u32 index, u64 event_word)
{
    ...
    if (type == 0x00U) {
        /* Peak event: [63:56]type [55:40]q [39:28]phase [27]pol [26:25]ch [24:0]seq. */
