/*
 * AXI DMA S2MM stream-consistency probe, v3 (2026-09-15).
 *
 * Why v3 exists
 * -------------
 * v2 measured correctly but had a side effect that cost board time.  It re-arms the
 * S2MM channel with no inter-transfer gap and polls DMASR flat out, so once Vitis
 * releases the CPU ("con") the target is left running at full load indefinitely.
 * On 2026-09-15 that produced this sequence, all from the same build:
 *
 *     11:45:06  dow <elf> ; 11:45:07 con    -> v2 ran, serial output observed  OK
 *     11:45:23  same ELF, rst -processor    -> Memory write error at 0x100000
 *     11:45:34+ every retry                 -> DAP status 0xF0000021
 *
 * The identical binary downloaded and ran seventeen seconds earlier, so the binary
 * was not the variable -- the state left on the target was.  Everything from the
 * second attempt onward is the sticky-error echo of that one failed bus transaction,
 * which is why retrying never helped and why the true first error is the only line
 * worth reading.
 *
 * v3 keeps the measurement identical, so its results stay comparable with the v2 run,
 * and adds only a bound: after PKT_LIMIT packets it prints the final report and parks
 * the CPU in WFI with the DMA no longer armed.  The bus goes quiet and the next
 * download starts from a clean target.
 *
 * The operational rule matters more than the code:
 *   ALWAYS halt the target (or power-cycle) before re-downloading.  Never press Run a
 *   second time against a board that is still executing the previous probe.  A
 *   continuously running instrument is indistinguishable, from the debugger's side,
 *   from a board that will not respond.
 *
...
/*
 * Bounded run.  v1/v2 ran forever; v3 stops after this many packets, prints the
 * final report and parks.  20000 packets is roughly 100 s at the observed ~200
 * packets/s cadence and is far more than the histogram and the accounting need.
 */
#define PKT_LIMIT         20000U
...
/*
 * Park the CPU with the S2MM channel left un-armed.  WFI stops the core from issuing
 * any further bus traffic, so the target goes quiet and the next download starts from
 * a clean state.  The PL event FIFOs will fill and back-pressure the feature core --
 * that is expected and harmless; no bus transaction is outstanding.
 */
#if defined(__arm__) || defined(__aarch64__)
#define CPU_PARK_STEP()   __asm__ __volatile__("wfi")
#else
#define CPU_PARK_STEP()   do { } while (0)
#endif
...
int main(void)
{
    ...
    xil_printf("\r\n--- S2MM stream-consistency probe v3 (BTT=%u, stop after %u packets) ---\r\n",
               S2MM_MAX_BTT, PKT_LIMIT);
    ...
    for (transfer = 0U; ; ++transfer) {
        ...
        /*
         * Bounded run: stop here rather than looping forever.  This is the whole point
         * of v3 -- a probe that never ends leaves the board in a running state that the
         * next debug session cannot recover from without a power cycle.
         */
        if (pkt_count >= PKT_LIMIT) {
            xil_printf("\r\n*** RUN COMPLETE: %u packets, %u bytes ***\r\n",
                       pkt_count, bytes_sum);
            print_report();
            xil_printf("PARKED: S2MM no longer re-armed, CPU in WFI.\r\n");
            xil_printf("Target is quiet; safe to halt and re-download.\r\n");
            park_forever();
        }
    }
}
