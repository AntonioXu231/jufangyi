...
static int receive_one_packet(void)
{
    ...
    for (i = 0U; i < DMA_POLL_LIMIT; ++i) {
        uart_poll();
#ifdef PD_ACQ_NETWORK_POLL
        /* The TCP service supplies this hook.  Keep it in the DMA wait loop
         * so STOP remains responsive while S2MM is waiting for TLAST. */
        PD_ACQ_NETWORK_POLL();
#endif
        if (g_stop_request) {
            XAxiDma_Reset(&g_dma);
